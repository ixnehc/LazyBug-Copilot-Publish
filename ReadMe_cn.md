# LazyBug - Visual Studio AI 编程助手扩展
...
- 新增 MCP 支持（stdio 和 URL）
- 新增 MCP 管理界面
- 在费用统计标签中新增缓存命中率显示

**Bug 修复**

- 修复了 Kimi 和 DeepSeek 在某些情况下 reasoning_content 未能正确返回给 LLM 的问题，该问题会导致复杂任务性能显著下降
- 修复了输出日志时可能崩溃的问题
- 当系统不支持 bash 时禁用 bash 工具
- 为 GLM API 启用了流式工具调用
- 修复了回滚所有历史对话后开启新对话会重置聊天标题的问题
- 修复了 Visual Studio 窗口回到前台时聊天输入窗口无法重新获得焦点的问题
- 聊天输入窗口重新获得焦点后，光标恢复到之前的位置
- 修复了使用某些端点时 token 统计数据会变得异常大的问题
- 修复了在超大文件中搜索关键词会失败的问题
...
- **符号搜索** — 对 C/C++/C# 代码进行快速符号搜索。开箱即用 — 无需额外配置 LSP。
...
- **上下文级别设置注意事项**：
  - 较高的上下文级别会消耗更多 token，产生更多费用，在昂贵模型上尤为明显。
  - 但较高的上下文级别会减少压缩频率，从而提高缓存命中率，从某种程度上也能降低成本。
